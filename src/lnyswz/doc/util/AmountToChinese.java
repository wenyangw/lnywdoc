...
public class AmountToChinese {
	...
	private final static String[] STR_MODIFY = { "", "拾", "佰", "仟", "万", "拾",
			"佰", "仟", "亿", "拾", "佰", "仟" };
	private static final String[] DOTUNIT = { "角", "分", "厘", "毫" };
	...
	private static String getFraction(String tempString) {
		...
		String dotPart = ""; // 取小数位
		int dotPos;
		if ((dotPos = tempString.indexOf('.')) != -1) {
			dotPart = tempString.substring(dotPos + 1);
		}
	...
	private static String dotToBig(String dotPart) {
		// 得到转换后的大写（小数部分）
		String strRet = "";
		for (int i = 0; i < dotPart.length() && i < 4; i++) {
			int num;
			num = Integer.parseInt(dotPart.substring(i, i + 1));
			strRet += STR_NUMBER[num];
			if (num == 0
					&& Integer.parseInt(dotPart.substring(i + 1, i + 2)) == 0) {
				i++;
			}
			if (num != 0) {
				strRet += DOTUNIT[i];
	...
	public static String numberToChinese(double tempNumber) {
		...
		// String amount="";
		// for(int i=0;i<sbResult.toString().length();i++){
		// amount += sbResult.toString().substring(i,i+1)+" ";
		// }
		return sbResult.toString();
	}
